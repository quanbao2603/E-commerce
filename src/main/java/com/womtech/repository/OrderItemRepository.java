...
public interface OrderItemRepository extends JpaRepository<OrderItem, String> {
 ...
 // Query cho doanh thu theo danh mục
    @Query("SELECT c.name, SUM(oi.price * oi.quantity) " +
           ...
           "WHERE oi.product.vendor.userID = :vendorId " +
 ...
 // Query cho top sản phẩm bán chạy
    @Query("SELECT p.name, SUM(oi.quantity) " +
           "FROM OrderItem oi " +
           "JOIN oi.product p " +
           "WHERE oi.product.vendor.userID = :vendorId " +
           "AND oi.order.createAt BETWEEN :start AND :end " +
    ...
    List<Object[]> findTopProductsByVendor(@Param("vendorId") String vendorId, 
                                          @Param("start") LocalDateTime start, 
                                          @Param("end") LocalDateTime end,
                                          Pageable pageable);
 // Query cho tổng doanh thu
    @Query("SELECT SUM(oi.price * oi.quantity) " +
           ...
           "AND oi.order.createAt BETWEEN :start AND :end " +
           "AND oi.order.status = 1")
    BigDecimal calculateTotalRevenueByVendor(@Param("vendorId") String vendorId, 
                                           @Param("start") LocalDateTime start, 
                                           @Param("end") LocalDateTime end);
    
    // Query cho tổng số đơn hàng
    @Query("SELECT COUNT(DISTINCT oi.order) " +
           "FROM OrderItem oi " +
           "WHERE oi.product.vendor.userID = :vendorId " +
           "AND oi.order.createAt BETWEEN :start AND :end " +
           "AND oi.order.status = 1")
    Long countDistinctOrdersByVendor(@Param("vendorId") String vendorId, 
                                   @Param("start") LocalDateTime start, 
                                   @Param("end") LocalDateTime end);
    
    // Query cho đếm đơn hàng theo trạng thái
    @Query("SELECT o.status, COUNT(DISTINCT oi.order) " +
           "FROM OrderItem oi " +
           "JOIN oi.order o " +
           "WHERE oi.product.vendor.userID = :vendorId " +
           "AND oi.order.createAt BETWEEN :start AND :end " +
