...
public class EmailUtil {
    ...
    public void sendVerifyOtp(String toEmail, String username, String otp, int ttlMinutes) {
        ...
        String content = Template.render(contentTpl, Map.of(
                "username", esc(username),
                "otp", esc(otp),
                "ttl", String.valueOf(ttlMinutes)
        ));
        String html = wrapInBase("Xác minh email – WOMTech", content, "https://www.facebook.com/jake.nguyen.3762", "Trang chủ");
        sendHtml(toEmail, "Xác minh email – WOMTech", html);
    }

    public void sendForgotPassword(String toEmail, String username, String otp, int ttlMinutes, @Nullable String resetLink) {
        String contentTpl = Template.load("mail/forgot-password.html");
        String content = Template.render(contentTpl, Map.of(
                "username", esc(username),
                "otp", esc(otp),
                "ttl", String.valueOf(ttlMinutes),
                "resetLink", nvl(resetLink)
        ));
        String html = wrapInBase("Khôi phục mật khẩu – WOMTech", content,
                nvl(resetLink), nvl(resetLink).isBlank() ? "" : "Đặt lại mật khẩu");
    ...
    public void sendWelcome(String toEmail, String username, @Nullable String ctaUrl) {
        ...
        String html = wrapInBase("Chào mừng đến với WOMTech!", content,
                nvl(ctaUrl), nvl(ctaUrl).isBlank() ? "" : "Khám phá ngay");
    ...
    public void sendOrderConfirmation(String toEmail,
                                      ...
                                      double totalAmount,
                                      @Nullable String deliveryEstimate,
        ...
        if (items == null || items.isEmpty()) {
            ...
            itemsTable = items.stream()
                    .map((OrderLine i) -> String.format(
                            "<tr>%s%s%s</tr>",
                            td(esc(i.name())),
                            td("x" + i.quantity()),
                            td(nf.format(i.price()))
                    ))
        ...
        String content = Template.render(contentTpl, Map.of(
                "username", esc(username),
                "orderCode", esc(orderCode),
                "deliveryEstimate", nvl(deliveryEstimate),
                "itemsTable", itemsTable,
                "total", nf.format(totalAmount)
        ));

        String html = wrapInBase("Xác nhận đơn hàng – WOMTech", content,
                nvl(trackUrl), nvl(trackUrl).isBlank() ? "" : "Theo dõi đơn hàng");
        sendHtml(toEmail, "Xác nhận đơn hàng #" + orderCode, html);
    }

    public void sendPromotion(String toEmail,
                              String title,
                              String message,
        ...
        String content = Template.render(contentTpl, Map.of(
                "title", esc(title),
                "message", esc(message),
                "promoCode", nvl(promoCode)
        ));

        String html = wrapInBase(title, content,
                nvl(ctaUrl), nvl(ctaUrl).isBlank() ? "" : "Mua ngay");
    ...
    private void sendHtml(String toEmail, String subject, String html) {
        try {
            ...
            if (html.contains("cid:appstoreBadge") || html.contains("cid:googlePlayBadge")) {
                ...
                if (appstore.exists()) helper.addInline("appstoreBadge", appstore, "image/png");
                if (gplay.exists())    helper.addInline("googlePlayBadge", gplay, "image/png");
            }
    ...
    private String wrapInBase(String title, String content, String buttonUrl, String buttonText) {
        ...
        return Template.render(base, Map.of(
                "TITLE", esc(title),
                "CONTENT", content,             
                "BUTTON_URL", esc(buttonUrl),
    ...
    static class Template {
        static String load(String classpathPath) {
            try (var in = EmailUtil.class.getClassLoader().getResourceAsStream(classpathPath)) {
                if (in == null) throw new IllegalArgumentException("Template not found: " + classpathPath);
